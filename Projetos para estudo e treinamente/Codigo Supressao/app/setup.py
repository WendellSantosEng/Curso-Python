...
from PyQt6.QtWidgets import QApplication,QGraphicsBlurEffect, QWidget, QLabel, QLineEdit, QPushButton, QFileDialog, QVBoxLayout, QSizePolicy, QMessageBox
...
class MinhaApp(QWidget):
    ...
    def initUI(self):
        ...
        label = QLabel(self)
        # Carregar a imagem usando QPixmap
        pixmap = QPixmap(r'Projetos para estudo e treinamente\\Codigo Supressao\\app\\arquea_logo.png')
        # Redimensionar a imagem para se ajustar proporcionalmente ao widget
        if not pixmap.isNull():
            # Redimensionar a imagem para se ajustar proporcionalmente ao widget
            pixmap = pixmap.scaled(self.width(), self.height(), aspectRatioMode=Qt.AspectRatioMode.KeepAspectRatio)
        ...
        # mensagem_label.setGeometry(100, 20, 200, 80)
        mensagem_label.setGeometry(130, 20, 140, 50)

        self.button_processar = QPushButton('Abrir documentos', self)
        self.button_processar.setGeometry(100, 70, 190, 50)
